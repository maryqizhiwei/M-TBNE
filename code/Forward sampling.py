...
df = pd.DataFrame.from_records(sampled_data)
...
print('prior_probabilities:', prior_probabilities)
#print('prior_probabilities:', prior_probabilities)
print(nodes_sort)
...
k = 1
S_ijk = pd.DataFrame(0, index=n_row_index, columns=n_row_index)
S_ijk = N_ijk.astype(float)
...
for edge in edges:
    for state_combination in itertools.product(*[range(network.get_cardinality(node)) for node in edge]):
        ...
        if N_ijk.loc[edge_0, edge_1] == 0:
            ...
            PMI_ijk.loc[edge_0, edge_1] = round(10 * edges_state_count[edge][state_combination] / size * np.log(N_ijk.loc[edge_0, edge_1]) - np.log(k), 3)
...
sampled_data_tensor = torch.tensor(sampled_data.values, dtype=torch.float32)
...
class MakePyGDataset(Dataset):
    ...
    def get(self, idx):
        ...
        #y = torch.tensor(self.sampled_data.iloc[idx].values, dtype=torch.long)
        #data = Data(x=x, edge_index=edge_index, y=y, edge_attr=edge_attr)


                # 随机遮掩节点特征
        num_nodes = len(sample_features_matrix)
        num_nodes_to_mask = int(0.2 * num_nodes)  # 例如，遮掩10%的节点
...
#print('dataset[0]:', dataset[0].x)
#print('dataset[0]:', dataset[0].edge_index)
#print('dataset[0]:', dataset[0].edge_attr)
