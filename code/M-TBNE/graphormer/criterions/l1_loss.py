...
class GraphPredictionL1Loss(FairseqCriterion):
    ...
    def forward(self, model, sample, reduce=True):
        ...
        with torch.no_grad():
            natoms = sample["net_input"]["batched_data"]["x"].shape[1] #得到节点数
        
        logits = model(**sample["net_input"]) #b*n*d dataset.x
        
        logits = logits[:, 0, :]
        #logits = logits[:, 1:, :]
        #print(logits.shape) #把virtual node 拿出 
        targets = model.get_targets(sample, [logits]) #b*n*d
        #print(targets.shape)
        loss = nn.L1Loss(reduction="sum")(logits, targets[: logits.size(0)]) #logits 
        #print('loss:', loss)
...
class GraphPredictionL1LossWithFlag(GraphPredictionL1Loss):
    ...
    def perturb_forward(self, model, sample, perturb, reduce=True):
        ...
        #print(logits.shape)
        loss = nn.L1Loss(reduction="mean")(logits, targets)
