...
def preprocess_item(item, max_N):
    edge_attr, edge_index, x = item.edge_attr, item.edge_index, item.x
    N = x.size(0)
    #x = convert_to_single_emb(x)
    # node adj matrix [N, N] bool
    #adj = torch.zeros([max_N, max_N], dtype=torch.bool)
    adj = torch.zeros([N, N], dtype=torch.bool)
    adj[edge_index[0, :], edge_index[1, :]] = True
    adj_weight = torch.zeros([N, N], dtype=torch.long)
    for i, edge in enumerate(edge_index.t()):
        adj_weight[edge[0], edge[1]] = edge_attr[i]
    # edge feature here
    if len(edge_attr.size()) == 1:
        edge_attr = edge_attr[:, None]
    attn_edge_type = torch.zeros([max_N, max_N, edge_attr.size(-1)], dtype=torch.long)
    #attn_edge_type[edge_index[0, :], edge_index[1, :]] = (convert_to_single_emb(edge_attr).long() + 1)

    shortest_path_result, path = algos.floyd_warshall(adj_weight.numpy())
    ...
    item.in_degree = item.in_degree[item.in_degree != 0] #去零
